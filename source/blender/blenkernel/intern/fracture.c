...
static void freeMeshIsland(MeshIsland *mi)
{
	if (mi->physics_mesh) {
...
static void free_meshislands(ListBase* meshIslands)
{
	MeshIsland *mi;
...
static void free_fracture_state(struct FractureState *fs, bool delete_shards)
{
	free_meshislands(&fs->island_map);
	if (delete_shards)
	{
...
static void free_fracture_container(struct FractureContainer *fc)
{
	FractureState *fs;
...
static void points_from_greasepencil(Object **ob, int totobj, Scene *scene, Object* obj, float thresh, FracPointCloud *points)
{
	bGPDlayer *gpl;
	bGPDframe *gpf;
	bGPDstroke *gps;
...
static FracPointCloud get_points_global(Scene* scene, Object *ob, ShardID id)
{
	...
	if (fc->point_source & MOD_FRACTURE_GREASEPENCIL && !(fs->flag & FM_FLAG_USE_GREASEPENCIL_EDGES)) {
		points_from_greasepencil(go, totgroup, scene, ob, thresh, &points);
	}
...
static void do_fracture(Scene *scene, Object *obj, ShardID id)
{
	...
	if (points.totpoints > 0 || (fc->flag & FM_FLAG_USE_GREASEPENCIL_EDGES)) {
		bool temp = fc->flag & FM_FLAG_SHARDS_TO_ISLANDS;
		short mat_index = 0;
		float mat[4][4];
		...
		BKE_fracture_create_dm(obj, true, false);
		if (temp)
			fc->flag |= FM_FLAG_SHARDS_TO_ISLANDS;
...
static void do_rigidbody(Scene *scene, Object *ob, MeshIsland* mi, short rb_type, int i)
{
	mi->rigidbody = NULL;
...
static void mesh_separate_tagged(Scene *scene, Object *ob, BMesh *bm_work)
{
	BMesh *bm_new;
...
static void do_constraint(Scene* scene, Object* ob, MeshIsland *mi1, MeshIsland *mi2, int con_type, float thresh)
{
	...
	rbsc = BKE_rigidbody_create_shard_constraint(scene, con_type);
	rbsc->mi1 = mi1;
	rbsc->mi2 = mi2;
...
static void connect_meshislands(Scene* scene, Object* ob, MeshIsland *mi1, MeshIsland *mi2, int con_type, float thresh)
{
	ConstraintContainer *cc = ob->fracture_constraints;
	int con_found = false;
	...
	if (!con_found && ok) {
		do_constraint(scene, ob, mi1, mi2, con_type, thresh);
	}
}

static void search_tree_based(Scene* scene, Object* ob, MeshIsland *mi, MeshIsland **meshIslands,
                              KDTree **combined_tree, GHash* vertex_island_map, float co[3])
{
	...
	for (i = 0; i < r; i++) {
		...
		if ((mi != mi2) && (mi2 != NULL)) {
			...
			connect_meshislands(scene, ob, mi, mi2, con_type, thresh);
		}
	}
...
static void create_constraints(Scene *scene, Object *ob, MeshIsland **mesh_islands, int count, KDTree *coord_tree, GHash* vertex_island_map, int target)
{
	FractureContainer *fc = ob->fracture_objects;
	FractureState *fs = fc->current;
	int i = 0;
	...
	for (i = 0; i < count; i++) {
		if (target == MOD_FRACTURE_CENTROID) {
			search_tree_based(scene, ob, mesh_islands[i], mesh_islands, &coord_tree, vertex_island_map, NULL);
		}
		else if (target == MOD_FRACTURE_VERTEX) {
			MVert mv;
			MeshIsland *mi = NULL;
			...
			search_tree_based(scene, ob, mi, mesh_islands, &coord_tree, vertex_island_map, mv.co);
		}
	}
}
...
void set_rigidbody_type(Object *ob, Shard *s, MeshIsland *mi)
{
	FractureContainer *fc = ob->fracture_objects;
	FractureState *fs = fc->current;
...
static void do_island_from_shard(Scene *scene, Object *ob, Shard* s, int i, int thresh_defgrp_index, int ground_defgrp_index, int vertstart)
{
	...
	//rb_type = do_vert_index_map(fmd, mi);
	do_rigidbody(scene, ob, mi, rb_type, i);
...
static MDeformVert* do_islands_from_shards(Scene* scene, Object* ob)
{
	...
	for (s = shardlist.first; s; s = s->next) {
		do_island_from_shard(scene, ob, s, i, thresh_defgrp_index, ground_defgrp_index, vertstart);
		vertstart += s->totvert;
		i++;
	}
...
static void do_clear(Object *ob)
{
	FractureContainer *fc = ob->fracture_objects;
	/* nullify invalid data */
	free_meshislands(&fc->current->island_map);
}
...
static void do_refresh(Scene* scene, Object *ob)
{
	double start = 0.0;
	MDeformVert *ivert = NULL;
	FractureContainer *fc = ob->fracture_objects;
	...
	//shards -> islands;
	ivert = do_islands_from_shards(scene, ob);
...
static void do_simulate(Scene* scene, Object *ob, DerivedMesh *dm, DerivedMesh *orig_dm)
{
	do_refresh(scene, ob);
...
static void add_fracture_state(Scene* scene, Object *ob, float frame)
{
	FractureContainer *fc = ob->fracture_objects;
...
DerivedMesh* BKE_fracture_create_dm(Object* ob, bool doCustomData, bool join_result)
{
	return create_dm(ob, doCustomData, join_result);
}
...
void BKE_build_constraints(Object *ob)
{
	...
	start = PIL_check_seconds_timer();
	create_constraints(ob, mesh_islands, count, coord_tree, vertex_index_map); /* check for actually creating the constraints inside*/
...
void BKE_prefracture_mesh(Object *ob)
{
	FractureContainer *fc = ob->fracture_objects;
	FractureState *fs = fc->current;
...
void BKE_dynamic_fracture_mesh(Object *ob)
{
	//test existence of ob->derivedDeform / ob->derivedFinal....
	do_modifier(ob, ob->derivedFinal);
...
void BKE_free_constraint_container(Object *ob)
{
	if (ob->fracture_constraints) {
		free_fracture_container(ob->fracture_constraints);
